...
"""
    ...
    def _pulisci_relazione(self, testo):
        ...
        # FIX "UN PO'" - NUCLEAR OPTION
        # Sostituisce tutte le varianti possibili, inclusi caratteri unicode strani
        testo = testo.replace("un pò", "un po'")
        testo = testo.replace("un po’", "un po'")
        testo = re.sub(r"un\s+p[\u00F2\u00F3]", "un po'", testo, flags=re.IGNORECASE) # ò ó unicode
        
        # Fix generico per varianti residue
        testo = re.sub(r"\bun\s+p[oòó]+(?:'|’)?\b", "un po'", testo, flags=re.IGNORECASE)
        testo = testo.replace("''", "'")  
