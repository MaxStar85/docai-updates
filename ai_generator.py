...
"""
    ...
    def _aggiungi_descrizione_denti(self, testo):
        ...
        # PASSAGGIO 1: Correggi/gestisci denti CHE HANNO GIA' la parentesi
        def _correggi_con_parentesi(match):
            prefisso = match.group(1)
            numero = match.group(2)
            descrizione_groq = match.group(3)
            dopo = match.group(4)
            
            numero_norm = numero.replace('.', '')
            
            # Se già descritto prima, rimuovi la parentesi (menzione successiva)
            if numero_norm in denti_gia_descritti:
                return f"{prefisso}{numero}{dopo}"
            
            denti_gia_descritti.add(numero_norm)
            
            # Cerca descrizione corretta
            descrizione_corretta = self.DESCRIZIONI_DENTI.get(
                numero, self.DESCRIZIONI_DENTI_NOPUNTO.get(numero_norm)
            )
            
            if not descrizione_corretta:
                return match.group(0)
            
            # Sostituisci SEMPRE con la descrizione corretta dalla tabella
        ...
        testo = re.sub(pattern_con, _correggi_con_parentesi, testo, flags=re.IGNORECASE)
        
        # PASSAGGIO 2: Aggiungi parentesi ai denti che NON ce l'hanno
        def _aggiungi_senza_parentesi(match):
            prefisso = match.group(1)
            numero = match.group(2)
            dopo = match.group(3)
            ...
            descrizione = self.DESCRIZIONI_DENTI.get(
                numero, self.DESCRIZIONI_DENTI_NOPUNTO.get(numero_norm)
            )
        ...
        testo = re.sub(pattern_senza, _aggiungi_senza_parentesi, testo, flags=re.IGNORECASE)
    ...
    def _pulisci_relazione(self, testo):
        ...
        # 0.5 AGGIUNGI/CORREGGI DESCRIZIONE DENTI
        ...
        # 0.65 RIMUOVI PSEUDO-TITOLI DI SEZIONE
        testo = re.sub(
            r"Nell'anamnesi del paziente,?\s*",
            '',
            testo
        )
        testo = re.sub(
            r"Nella sua anamnesi,?\s*(il Sig\.|la Sig\.ra|il paziente|la paziente)?\s*",
            'Il paziente ',
            testo,
            flags=re.IGNORECASE
        )
        ...
        testo = re.sub(
            r"Durante l'esame obiettivo,?\s*",
            "L'esame obiettivo ha rivelato ",
            testo
        )
        testo = re.sub(
            r"All'esame obiettivo,?\s*",
            "L'esame obiettivo ha rivelato ",
            testo
        )
        testo = re.sub(
            r"Per quanto riguarda la diagnosi,?\s*",
            '',
            testo
        )
        testo = re.sub(
            r"Il piano di trattamento proposto prevede\s*",
            'Si procederà con ',
            testo
        )
        # Fix: "Il paziente il Sig." → "Il Sig."
        testo = re.sub(
            r"Il paziente il (Sig\.|Sig\.ra)\s*",
            r"Il \1 ",
            testo
        )
        # Fix: minuscola a inizio paragrafo dopo rimozione pseudo-titolo
        testo = re.sub(
            r"\n([a-z])",
            lambda m: "\n" + m.group(1).upper(),
            testo
        ...
        # 0.7 FIX SPIEGAZIONI TECNICHE ACCUMULATE
        testo = re.sub(
            r'(?:trattamento |terapia )?endodontic[oa]'
            r'(?:\s*,|\s)\s*(?:ovvero|cioè)\s+(?:una?\s+)?(?:terapia |cura )?canalare'
            r'(?:\s*,\s*(?:cioè|ovvero)\s+[^.;]+)?',
        ...
        testo = re.sub(
            r'la polpa\s*,?\s*(?:cioè |ovvero )?(?:il |del )?nervo(?:\s+del\s+dente)?',
            'la polpa (il nervo)',
            testo,
            flags=re.IGNORECASE
        )
        testo = re.sub(
            r'(terapia (?:endodontica|canalare) \([^)]+\))'
            r'\s*,\s*(?:cioè|ovvero)\s+[^.;]+',
            r'\1',
            testo,
            flags=re.IGNORECASE
        ...
        # "un terapia" → "una terapia", "un corona" → "una corona", ecc.
        testo = re.sub(
        ...
        # "per il terapia" → "per la terapia" (articolo sbagliato)
        testo = re.sub(
